...
function StudentDashboard() {
  ...
  return <DashboardFrame eyebrow="Student dashboard" title="Keep your momentum visible." stats={[["Active paths", String(items.length), BookOpenCheck], ["Certificates", String(certificates.data?.length ?? 0), Award], ["Completed courses", String(items.filter(item => item.status === 'completed').length), Clock3]]}>
...
function InstructorDashboard() {
  ...
  return <DashboardFrame eyebrow="Instructor studio" title="Shape knowledge people can use." stats={[["Your courses", String(items.length), BookOpenCheck], ["Published", String(items.filter((course) => course.status === 'published').length), CheckCircle2], ["Awaiting review", String(items.filter((course) => course.status === 'pending').length), BarChart3]]}>
    <p className="mb-4">New instructors require approval before creating courses. Status: {user?.instructorProfile?.approvalStatus ?? 'refresh this page to check'}.</p><div className="flex items-end justify-between"><div><h2 className="text-3xl">Course studio</h2><p className="mt-2 font-sans text-sm text-[#657066]">Draft, submit, and refine your courses.</p></div><Link to="/instructor/courses/new" className="flex items-center gap-2 rounded-full bg-[#d86445] px-4 py-2 font-sans text-xs font-bold text-white"><FilePlus2 size={15} /> New course</Link></div>
    <div className="mt-6 grid gap-4 md:grid-cols-2">{items.length ? items.map((course) => <div key={course.id} className="border border-[#d8d2c3] bg-[#fbfaf5] p-5"><div className="flex items-start justify-between gap-4"><h3 className="text-xl">{course.title}</h3><span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#d86445]">{course.status}</span></div><p className="mt-2 font-sans text-sm text-[#657066]">{course.shortDescription}</p><Link className="mt-5 block underline" to={`/instructor/courses/${course.id}`}>Open course workspace</Link></div>) : <EmptyState title="No courses yet" detail="Create your first course to begin teaching." />}</div>
  </DashboardFrame>;
}
...
function AdminDashboard() {
  ...
  return <DashboardFrame eyebrow="Admin control room" title="Keep the learning system healthy." stats={[["Pending courses", String(items.length), ShieldCheck], ["Review queue", String(items.length), Users], ["Management", "Instructors and categories", BarChart3]]}>
    <Link to="/admin/manage" className="mb-5 block underline">Manage instructors and categories</Link>{review.isError && <ErrorState message={apiErrorMessage(review.error)} />}<div><h2 className="text-3xl">Course approvals</h2><p className="mt-2 font-sans text-sm text-[#657066]">Review instructor submissions before they reach students.</p></div>
    <div className="mt-6 space-y-3">{items.length ? items.map((course) => <div key={course.id} className="flex flex-col gap-4 border border-[#d8d2c3] bg-[#fbfaf5] p-5 sm:flex-row sm:items-center"><div className="flex-1"><h3 className="text-xl"><Link to={`/instructor/courses/${course.id}`}>{course.title} - inspect content</Link></h3><p className="mt-1 font-sans text-sm text-[#657066]">{course.shortDescription}</p></div><div className="flex gap-2"><button disabled={review.isPending} onClick={() => review.mutate({ id: course.id, action: 'reject' })} className="border border-[#d86445] px-3 py-2 font-sans text-xs font-bold text-[#d86445]">Reject</button><button disabled={review.isPending} onClick={() => review.mutate({ id: course.id, action: 'approve' })} className="bg-[#193b31] px-3 py-2 font-sans text-xs font-bold text-white">Approve</button></div></div>) : <EmptyState title="Nothing to review" detail="New course submissions will appear here." />}</div>
  </DashboardFrame>;
}
...
export function LearningPage() {
  ...
  const client = useQueryClient();
  const complete = useMutation({ mutationFn: (lessonId: number) => studentApi.completeLesson(lessonId), onSuccess: async () => { await Promise.all(['progress', 'enrollments', 'certificates', 'notifications'].map(key => client.invalidateQueries({ queryKey: [key] }))); } });
  if (enrollments.isLoading || progress.isLoading) return <LoadingState />;
  if (enrollments.isError || progress.isError) return <ErrorState message="Learning progress could not load." />;
  if (!enrollment?.course) return <ErrorState message="This learning path could not be found." />;
  const lessons = [...(enrollment.course.modules ?? [])].sort((a,b) => a.position-b.position).flatMap((module) => [...(module.lessons ?? [])].sort((a,b) => a.position-b.position)) ?? [];
  return <section className="mx-auto max-w-7xl px-5 py-12 lg:px-8"><Link to="/dashboard" className="font-sans text-sm text-[#657066]">← Dashboard</Link><div className="mt-7 flex flex-col justify-between gap-6 border-b border-[#d8d2c3] pb-8 md:flex-row md:items-end"><div><p className="font-sans text-xs font-bold uppercase tracking-[.25em] text-[#d86445]">Learning path</p><h1 className="mt-2 text-5xl">{enrollment.course.title}</h1></div><div className="min-w-56"><div className="flex justify-between font-sans text-xs"><span>Course progress</span><strong>{progress.data?.percentage ?? 0}%</strong></div><div className="mt-2 h-2 bg-[#d8d2c3]"><div className="h-full bg-[#d86445]" style={{ width: `${progress.data?.percentage ?? 0}%` }} /></div></div></div><div className="mt-9 grid gap-3">{lessons.map((lesson) => { const done = progress.data?.progress.some((item) => item.lessonId === lesson.id && item.completed); return <div key={lesson.id} className="flex items-center gap-4 border border-[#d8d2c3] bg-[#fbfaf5] p-5"><div className={`grid h-10 w-10 place-items-center ${done ? 'bg-[#d86445] text-white' : 'bg-[#e9e4d7] text-[#657066]'}`}>{done ? <CheckCircle2 size={18} /> : <BookOpenCheck size={18} />}</div><div className="flex-1"><h3 className="text-xl">{lesson.title}</h3><p className="mt-3 whitespace-pre-wrap font-sans leading-7">{lesson.content}</p>{lesson.videoUrl && <a className="mt-3 block underline" href={lesson.videoUrl} target="_blank" rel="noreferrer">Watch lesson video</a>}<p className="mt-1 font-sans text-xs text-[#657066]">{lesson.durationMinutes ? `${lesson.durationMinutes} min` : 'Lesson'} · {done ? 'Completed' : 'Ready to learn'}</p></div>{!done && <button disabled={complete.isPending} onClick={() => complete.mutate(lesson.id)} className="rounded-full bg-[#193b31] px-4 py-2 font-sans text-xs font-bold text-white">Mark complete</button>}</div>; })}</div>{complete.isError && <ErrorState message={apiErrorMessage(complete.error)} />}<div className="mt-6 space-y-3">{enrollment.course.quizzes?.map(quiz => <Link className="block underline" key={quiz.id} to={`/quizzes/${quiz.id}`}>Take quiz: {quiz.title}</Link>)}</div><CourseReviews courseId={enrollment.course.id} canReview /><p className="mt-6 font-sans text-xs text-[#657066]">Complete every lesson to unlock your certificate.</p></section>;
}

export function CertificatesPage() { const query = useQuery({ queryKey: ['certificates'], queryFn: () => studentApi.certificates().then((r) => r.data.data) }); if (query.isLoading) return <LoadingState />; if (query.isError) return <ErrorState message={apiErrorMessage(query.error)} />; return <section className="mx-auto max-w-5xl px-5 py-14 lg:px-8"><p className="font-sans text-xs font-bold uppercase tracking-[.25em] text-[#d86445]">Proof of progress</p><h1 className="mt-3 text-5xl">Certificates</h1><button className="mt-4 underline print:hidden" onClick={() => window.print()}>Print or save certificates as PDF</button><div className="mt-9 space-y-3">{query.data?.length ? query.data.map((certificate) => <div key={certificate.id} className="flex items-center gap-4 border border-[#d8d2c3] bg-[#fbfaf5] p-5"><Award className="text-[#d86445]" /><div className="flex-1"><h2 className="text-xl">{certificate.course?.title}</h2><p className="font-sans text-xs text-[#657066]">Issued {formatDate(certificate.issuedAt)} · {certificate.certificateNumber}</p></div><span className="font-sans text-xs font-bold uppercase tracking-widest text-[#d86445]">Earned</span></div>) : <EmptyState title="No certificates yet" detail="Finish every lesson in a course to earn one." />}</div></section>; }

export function NotificationsPage() { const query = useQuery({ queryKey: ['notifications'], queryFn: () => studentApi.notifications().then((r) => r.data.data) }); const client = useQueryClient(); const mark = useMutation({ mutationFn: (id: number) => studentApi.markNotificationRead(id), onSuccess: () => client.invalidateQueries({ queryKey: ['notifications'] }) }); if (query.isLoading) return <LoadingState />; if (query.isError) return <ErrorState message={apiErrorMessage(query.error)} />; return <section className="mx-auto max-w-3xl px-5 py-14 lg:px-8"><p className="font-sans text-xs font-bold uppercase tracking-[.25em] text-[#d86445]">Your feed</p><h1 className="mt-3 text-5xl">Notifications</h1>{mark.isError && <ErrorState message={apiErrorMessage(mark.error)} />}<div className="mt-9 space-y-3">{query.data?.length ? query.data.map((item) => <div key={item.id} className={`border p-5 ${item.readAt ? 'border-[#d8d2c3] bg-transparent' : 'border-[#d86445] bg-[#fffaf1]'}`}><div className="flex justify-between gap-4"><h2 className="text-xl">{item.title}</h2>{!item.readAt && <button disabled={mark.isPending} onClick={() => mark.mutate(item.id)} className="font-sans text-xs font-bold text-[#d86445]">Mark read</button>}</div><p className="mt-2 font-sans text-sm leading-6 text-[#657066]">{item.message}</p></div>) : <EmptyState title="All caught up" detail="New course and progress updates will appear here." />}</div></section>; }
