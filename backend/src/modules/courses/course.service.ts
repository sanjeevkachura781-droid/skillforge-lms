...
import { Category, Course, CourseLevel, CourseStatus, User } from '../../database/models/index.js';
...
export async function listPublishedCourses(input: { page: number; limit: number; search?: string; categoryId?: number; level?: CourseLevel }) {
  ...
  const result = await Course.findAndCountAll({ where, include: courseIncludes, order: [['createdAt', 'DESC']], limit, offset: (page - 1) * limit, distinct: true });
...
export async function getPublishedCourse(slug: string) {
  const course = await Course.findOne({ where: { slug, status: CourseStatus.PUBLISHED }, include: [{ ...courseIncludes[0], required: false }, { ...courseIncludes[1], required: false }, { association: 'modules', include: [{ association: 'lessons' }] }] });
